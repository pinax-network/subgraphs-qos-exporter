...
// Per-indexer detail is high-cardinality, so by default we emit it only for tracked deployments
// (those in deployments.json — the subgraphs we care about ranking on). Set INDEXER_DETAIL=all to
// emit per-indexer series for EVERY deployment network-wide (thousands of series).
const INDEXER_DETAIL_ALL = (process.env.INDEXER_DETAIL ?? "tracked").toLowerCase() === "all";
